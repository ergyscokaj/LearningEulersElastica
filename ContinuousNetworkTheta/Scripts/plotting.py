...
def plotTestResults(model,device,number_elements,number_components,x_train,x_test,y_train,y_test):
    ...
    res = reconstruct_q(q1,q2,v1,v2,model,device)
...
def compute_errors(model,device,number_elements,number_components,x_train,x_test,x_val,y_train,y_test,y_val):
    ...
    res_test = reconstruct_q(q1,q2,v1,v2,model,device)
    ...
    res_train = reconstruct_q(q1,q2,v1,v2,model,device)

    #Validation trajectories
    validation_trajectories = np.concatenate((x_val[:,:4],y_val,x_val[:,-4:]),axis=1)
    bcs = getBCs(validation_trajectories)
    q1 = bcs["q1"]
    q2 = bcs["q2"]
    v1 = bcs["v1"]
    v2 = bcs["v2"]
    ...
    res_derivative_val = np.zeros((len(validation_trajectories),2,len(xx)))
    for j in range(len(validation_trajectories)):
        for i in range(len(xx)):
            res_derivative_val[j,:,i] = eval_derivative_model(model,device,xx[i],q1[j],q2[j],v1[j],v2[j])

    res_val = reconstruct_q(q1,q2,v1,v2,model,device)
    ...
    q_val_pred = res_val.transpose(0,2,1).reshape(len(res_val),-1)
    ...
    v_val_pred = res_derivative_val.transpose(0,2,1).reshape(len(res_derivative_val),-1)

    pred_train_all = np.zeros_like(training_trajectories)
    pred_test_all = np.zeros_like(test_trajectories)
    pred_val_all = np.zeros_like(validation_trajectories)

    pred_train_all[:, q_idx] = q_train_pred
    pred_test_all[:, q_idx] = q_test_pred
    pred_val_all[:, q_idx] = q_val_pred
    pred_train_all[:, v_idx] = v_train_pred
    pred_test_all[:, v_idx] = v_test_pred
    pred_val_all[:, v_idx] = v_val_pred
    
    train_error = np.mean((pred_train_all-training_trajectories)**2)
    test_error = np.mean((pred_test_all-test_trajectories)**2)
    val_error = np.mean((pred_val_all-validation_trajectories)**2)

    return test_error, train_error, val_error, pred_train_all, pred_test_all, pred_val_all